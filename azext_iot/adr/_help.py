...
def load_adr_help():
  ...
  long-summary: |
    By default, policies use a service-managed CA. To use your own CA (Bring Your Own Root),
    use --enable-byor and then activate the policy with the signed CSR using activate-byor.
  examples:
  ...
  long-summary: |
    For BYOR (Bring Your Own Root) policies, the output includes the certificate signing request (CSR)
    that needs to be signed by your Certificate Authority.
  examples:
  ...
  long-summary: |
    This command revokes the current CA (issuer) certificate for the policy, triggering the service
    to generate a new CA certificate. All device credentials issued by the old CA will need to be
    re-issued. Use this when the CA certificate is compromised or needs rotation.
  examples:
  ...
  long-summary: |
    This command activates a BYOR policy by uploading the signed certificate chain. Use this after
    creating a policy with --enable-byor and signing the CSR (from 'policy show') with your CA.
