...
def test_create_registry_device(fixture_registry_device_provider):
    """Create resolves location from the namespace and builds the properties body (non-LRO)."""
    sentinel = {"name": "dev"}
    fixture_registry_device_provider.client.registry_devices.create_or_replace.return_value = sentinel
    ...
    call = fixture_registry_device_provider.client.registry_devices.create_or_replace.call_args[1]
...
def test_create_registry_device_uses_explicit_location(fixture_registry_device_provider):
    """Create skips namespace lookup when location is supplied explicitly."""
    fixture_registry_device_provider.client.registry_devices.create_or_replace.return_value = {}
...
def test_update_registry_device(fixture_registry_device_provider):
    """Update sends only the provided properties (non-LRO) and returns the response."""
    sentinel = {"name": "dev"}
    fixture_registry_device_provider.client.registry_devices.update.return_value = sentinel
    ...
    assert result == sentinel
    props = fixture_registry_device_provider.client.registry_devices.update.call_args[1]["properties"]
...
def test_create_registry_device_with_tags(fixture_registry_device_provider):
    ...
    resource = fixture_registry_device_provider.client.registry_devices.create_or_replace.call_args[
        1
    ]["resource"]
    assert resource["tags"] == {"env": "prod"}


def test_update_registry_device_with_tags(fixture_registry_device_provider):
    """Tags-only update sends tags in the patch body."""
    fixture_registry_device_provider.client.registry_devices.update.return_value = {"name": "dev"}
    ...
    body = fixture_registry_device_provider.client.registry_devices.update.call_args[1]["properties"]
